...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map, usually `.github/skills/verify-*/`. If several candidates exist, use `ask_user` to choose one. If none exists, stop and point at `/create-verification-skill`.
