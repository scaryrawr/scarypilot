...
- Existing-skill-first: propose `new skill via authoring playbook:` only when no existing skill is a real home, the pattern recurs, and the topic deserves its own skill.
...
| <new pattern, no existing skill is a real home> | <draft a new Agent Skill> | <new skill via authoring playbook: <kebab-name>> |
