...
- `agent_type`: `explore`
- `model`: your configured how-explorer model; omit it when unconfigured or set to `auto`
...
- `model`: your configured how-explainer model; omit it when unconfigured or set to `auto`
...
After the explanation is complete, spawn one architectural critic per model in the configured how-critics list. Without a configuration, spawn two critics and omit `model`.

For each critic:
- `agent_type`: `general-purpose`
- `model`: one model from the configured how-critics list; omit it when unconfigured or set to `auto`
- Prompt: read-only; do not edit files
