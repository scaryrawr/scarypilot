...
1. Spawn a `general-purpose` Task agent with an explicitly read-only prompt. Tell it to read the linked [`comment-sicko`](../../agents/comment-sicko.agent.md) agent definition, then pass the scope. If custom plugin agents are exposed by name, `comment-sicko` is equivalent.
