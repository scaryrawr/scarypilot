...
State your read of scope and constraints in one paragraph. Use `ask_user` only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Prefer the `poteto-agent` custom agent when Task advertises it. Otherwise use `agent_type: "general-purpose"` and tell it to read `poteto-mode`.
- Pass `model` only when the project configuration names an available model; omit it for `auto`.
...
- Browser / Electron / Web UIs: a project verification skill or the available browser and computer-use tools.
- CLIs and TUIs: a project verification skill or the available terminal and shell tools.
...
- the repository formatter and linter over each diff before commit, and the **unslop** skill over every prose surface.
