...
- **Coordinator (this chat).** Frames, authors briefs, drains the inbox, owns the human report, and makes judgment calls. It never authors or edits code: conflicted merges, restacks, and code changes are tasks. Mechanically landing a verified unit is bookkeeping the coordinator may do itself. Agents are started and observed through Copilot's Task and agent tools. State reads and writes go through the bundled `scripts/orch/orch.ts` at drain points.
- **Sub-coordinator.** One per track only when the program exceeds what one coordinator can drain. It owns its track's units and boards, authors briefs, and rolls up aggregates at wave boundaries. Let it spawn workers only when the active Task schema supports nested agents; otherwise the parent coordinator spawns those workers from the returned briefs. Cap in-flight children at what one drain can process, roughly ten, as a rolling window.
- **Worker / verifier.** Use Task agents for bounded units and app child sessions when the current Copilot host exposes them. Briefs inline required context or point at repository paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different configured model family from its worker when available.
...
Resolve a durable local store with `git rev-parse --git-path pstack/orchestrate/<project-slug>`. Resolve the bundled script relative to this skill's directory. Every command below written as `orch --store <store>` means `bun <poteto-mode-skill>/scripts/orch/orch.ts --store <resolved-store>`. Every file has exactly one writer; owners publish facts, readers aggregate at read time.
...
- `inbox/` holds completion pointers. `gates.md` parks human gates (question, options, default on no answer) so a completion flood cannot wipe ask_user state.
...
2. **Install the runtime.** Run `orch --store <store> init`. Open the trail via the show-me-your-work skill, write the standing orders before any spawn, and seed `frontier.json` from existing PRs with `orch --store <store> frontier set --repo <repo-dir>`.
...
- On a completion notification, run `orch --store <store> inbox push <agent> <unit> <status> [--report PATH]` and return to what you were doing. Never deep-review inline; a completion that needs review becomes a verifier unit.
- Drain in batches at four points: the end of a critical section, a track rollup, a frontier watcher notification, and before a human report. Begin each batch with `orch --store <store> inbox drain`. Arrivals during a drain wait for the next one.
...
- Each drain classifies every pointer (landed, needs-verify, failed, zombie, noise), writes the resulting rows through `orch --store <store> unit add`, `unit set`, and `ledger record`, runs `orch --store <store> status`, then spawns the next wave in one message.
...
- A drain turn ends with the three lines from `orch --store <store> status`: counts against the states, what changed, gates open. Detail lives in `status.md`; the full reply contract applies at checkpoints and close.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack; record the holder in the standing orders. Run restacks in an isolated worktree and keep them serialized.
...
Write ledger rows with `orch --store <store> ledger record`. Check the current PR and head SHA with `orch --store <store> ledger check`. `ledger.tsv`, one row per verdict, keyed by PR number plus head SHA: `live-ui-verified | unit-test-verified | type-check-only | verifier-blocked | verifier-failed`. CI green is an input to a verdict, not a verdict. Behavioral work needs better than `type-check-only`. `verifier-blocked` is not a pass; respawn when the environment heals. `verifier-failed` gets a fix unit, not a re-verify. A worker may self-report; a verifier overrides it on the same key. A new head SHA voids the row, so re-verify after restack.

A unit is not done until its output is externalized the moment it lands: a worker pushes its branch, a verifier writes its ledger row, and receipts land in the store. Work that exists only in an agent process was never done.

#### Liveness and failure

- Never message or resume an agent merely to check on it. Probe with `read_agent`, the ledger, `units.tsv`, `gh`, and pushed branches. Session timestamps are not liveness.
...
- After a Copilot session restart, assume Task agents stopped. Re-read the standing orders and `units.tsv`, recompute the frontier, recover durable work by PR and branch rather than agent ID, respawn needed coordinators from stored briefs, drain, and resume. `orch` replaces a stale lock whose holder process is gone.
