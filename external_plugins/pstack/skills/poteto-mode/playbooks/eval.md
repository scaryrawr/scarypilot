...
6. **Verify the chain from session evidence, not self-report.** Use Task results or query only the candidate's Copilot session when session tools are available. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the evidence plus the shape of the code, never from the candidate's own claims.
