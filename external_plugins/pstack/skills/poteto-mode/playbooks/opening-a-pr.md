...
**PRs.** Run `/deslop` over the diff before commit. It applies `/anti-slop` to changed TypeScript and JavaScript. Then run the repository's formatter and linter. Run `/no-comments` before review. Write every PR title, PR description, and commit body with `/technical-writing`, then apply `/unslop`. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.
