...
**Use the `poteto-agent` custom agent for playbook delegates when the Task tool advertises it.** Otherwise use `agent_type: "general-purpose"` and tell the delegate to read the `poteto-mode` skill before starting. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) set their own agent types; respect what the skill prescribes.
...
- **Autopilot-stack.** A queue of changes built and verified with full autonomy, delivered as one linear reviewed GitHub base-branch stack the operator lands herself ("autopilot-stack", "stack them, don't ship", "build the stack, I'll land it"). `playbooks/autopilot-stack.md`.
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a Copilot session, session URL, or pushed branch. `playbooks/session-pickup.md`.
- **Pause safely.** Suspending in-flight work cleanly so it can be resumed, on an explicit pause, going offline, a Copilot session restart, or imminent context compaction. The complement to Session pickup. Full steps: `playbooks/pause-safely.md`.
