...
- **Python REPL**: `tmux ... send-keys -- 'PYTHON_BASIC_REPL=1 python3 -q' Enter`; wait for `^>>>`; send code with `-l`; interrupt with `C-c`. Always with `PYTHON_BASIC_REPL`.
...
- Kill all sessions on a socket: `tmux -S "$SOCKET" list-sessions -F '#{session_name}' | while IFS= read -r s; do tmux -S "$SOCKET" kill-session -t "$s"; done`.
...
- `-S`/`--socket` tmux socket path (use `"$SOCKET"` to target the private socket)
