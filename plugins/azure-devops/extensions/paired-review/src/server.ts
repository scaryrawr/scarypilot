...
const ServerAddressSchema = Type.Object({ port: Type.Number() });
...
export async function startReviewServer(options: ReviewServerOptions) {
  ...
  const address = Value.Parse(ServerAddressSchema, server.address());
...
function parseThreadInput(body: JsonValue): CreateReviewThreadInput | null {
